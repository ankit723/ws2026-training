        ...
        <div className="w-screen h-screen flex justify-center items-center p-10">
            <div className="w-full h-full grid grid-cols-7 grid-rows-7">
                {["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"].map(day=>(
                    <div key={day} className="border border-blue-400 flex justify-center items-center">
                        {day}
                    </div>
                ))}
                {
                    Array.from({length: firstDay}).map((d, i)=>(
                        <div key={i} className="border border-blue-400 flex justify-center items-center"></div>
                    ))
                }
                {
                    daysArray.map((day) => (
                        <div key={day} className="border flex justify-center items-center">
                            {day}
                        </div>
                    ))
                }
